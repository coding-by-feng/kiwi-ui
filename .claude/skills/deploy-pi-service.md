...
- Host: `kason-pi.local`
- User: `kason`
- Auth: `sshpass -p 'changeme' ssh kason@kason-pi.local`
- Node: `/usr/bin/node`
...
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "sudo systemctl status <service-name>"
```

### Restart a service
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "sudo systemctl restart <service-name> && sleep 1 && sudo systemctl status <service-name>"
```

### View logs
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "sudo journalctl -u <service-name> -n 50 --no-pager"
```
...
```bash
sshpass -p 'changeme' scp <local-script-path> kason@kason-pi.local:/home/kason/kiwi-ui/scripts/
```

2. **Install dependencies** (if needed):
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "cd /home/kason/kiwi-ui && npm install <package-name>"
```

3. **Create systemd service file**:
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "cat << 'EOF' | sudo tee /etc/systemd/system/<service-name>.service
[Unit]
...
4. **Enable and start**:
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "sudo systemctl daemon-reload && sudo systemctl enable <service-name> && sudo systemctl start <service-name> && sleep 2 && sudo systemctl status <service-name>"
```
...
6. **Reload nginx**:
```bash
sshpass -p 'changeme' ssh kason@kason-pi.local "sudo nginx -t && sudo systemctl reload nginx"
```
